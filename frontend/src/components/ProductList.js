...
const ProductList = () => {
  ...
  const searchHandle = async (event) => {
    ...
    } else {// if nothing is provided in the search box then call the getProducts(), i.e. get all the available products
      getProducts();
    }
  };
      ...
      {products.length > 0 ? (
        products.map((item, index) => (
          <ul key={item._id}>
            ...
            <li>{item.category}</li>
            <li>
              <Link className="update-button" to={"/update/" + item._id}><FontAwesomeIcon icon={faWrench} /></Link>
              <p>   /  </p>
              <button className="delete-button" onClick={() => deleteProduct(item._id)}> <FontAwesomeIcon icon={faTrashCan} /></button>
            </li>
          </ul>
        ))
      ) : (
